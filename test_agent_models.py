...
print("\n1. ToolCall Model:")
tool_call_json = generate_json_example_from_pydantic(ToolCall)
print(f"JSON example: {tool_call_json}")
# Parse and pretty print
parsed = json.loads(tool_call_json)
print("Parsed and formatted:")
print(json.dumps(parsed, indent=2))

print("\n2. AgentThought Model:")
agent_thought_json = generate_json_example_from_pydantic(AgentThought)
print(f"JSON example: {agent_thought_json}")
# Parse and pretty print
parsed = json.loads(agent_thought_json)
print("Parsed and formatted:")
print(json.dumps(parsed, indent=2))
...
print("\n1. ToolCall Model:")
tool_call_prompt = """
Generate a tool call to search for information about Python programming.
The tool should be named 'search' and include appropriate parameters.
"""
try:
...
Generate an agent's thought process about solving a math problem.
"""
try:
    result = generate_pydantic_json_model(
        model_class=AgentThought,
        prompt=agent_thought_prompt,
        llm_instance=llm,
        max_retries=1,
        full_response=True
    )
...
print("\n5. AgentResponse Model:")
agent_response_prompt = """
Generate a complete agent response with a thought process.
The agent should think about how to respond to a user asking about the weather.
"""
try:
    result = generate_pydantic_json_model(
        model_class=AgentResponse,
