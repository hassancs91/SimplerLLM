/**
 * YouTube Thumbnail Generator Manager - Handles thumbnail generation UI and logic
 */
class YouTubeThumbnailManager {
    constructor() {
        ...
        // DOM elements
        this.facePreview = document.getElementById('thumbnail-face-preview');
        this.resultPreview = document.getElementById('thumbnail-result-preview');
        this.promptInput = document.getElementById('thumbnail-prompt-input');
        this.textInput = document.getElementById('thumbnail-text-input');
        this.modelSelect = document.getElementById('thumbnail-model-select');
        this.generateBtn = document.getElementById('btn-generate-thumbnail');
        this.statusSection = document.getElementById('thumbnail-status');
        this.presetsContainer = document.getElementById('thumbnail-presets');
        this.browseBtn = document.getElementById('btn-browse-face-image');
        this.galleryBtn = document.getElementById('btn-select-face-gallery');
        this.galleryPickerModal = document.getElementById('thumbnail-gallery-picker-modal');
        this.galleryPickerGrid = document.getElementById('thumbnail-gallery-picker-grid');
        this.closePickerBtn = document.getElementById('btn-close-thumbnail-gallery-picker');
        this.configureApiKeyBtn = document.getElementById('btn-configure-api-key-thumbnail');
    ...
    setupEventListeners() {
        ...
        // Update generate button state when inputs change
        if (this.promptInput) {
            this.promptInput.addEventListener('input', () => this.updateGenerateButtonState());
        }
        if (this.textInput) {
            this.textInput.addEventListener('input', () => this.updateGenerateButtonState());
        }
    }
    ...
    async loadPresets() {
        const result = await api.getThumbnailPresets();
    ...
    async openGalleryPicker() {
        ...
        this.galleryPickerGrid.innerHTML = result.images.map(img => `
            <div class="gallery-picker__item" data-id="${img.id}">
                ...
                ${img.type === 'thumbnail' ? '<span class="image-badge image-badge--thumbnail">Thumbnail</span>' : ''}
    ...
    updateGenerateButtonState() {
        if (this.generateBtn) {
            // Face reference is optional, but prompt or preset is required
    ...
    async generate() {
        ...
        const thumbnailText = this.textInput?.value.trim();

        if (!prompt && !this.selectedPreset) {
        ...
        if (this.statusSection) {
            ...
            this.statusSection.innerHTML = `
                <div class="generation-status__header">
                    <span class="badge badge--warning">Creating Thumbnail</span>
                </div>
                ...
                <p style="font-size: var(--font-size-sm); color: var(--text-secondary); margin-top: var(--spacing-sm);">
                    AI is generating your YouTube thumbnail...
                </p>
            `;
        }
    }
    ...
    showResult(result) {
        ...
        this.resultPreview.innerHTML = `
            <h3 class="section-title">Generated Thumbnail</h3>
            <div class="thumbnail-result">
                <img class="thumbnail-result__image" src="${api.getImageUrl(result.image_id)}" alt="Generated thumbnail">
                <div class="thumbnail-result__info">
                    <span class="badge badge--success">16:9 YouTube Thumbnail</span>
                    ${result.preset_used ? `<span class="badge badge--secondary">${this.getPresetName(result.preset_used)}</span>` : ''}
                </div>
            </div>
            <div class="thumbnail-actions">
                <button class="btn btn--secondary btn--sm" id="btn-generate-another">
                    Generate Another
                </button>
                <button class="btn btn--primary btn--sm" id="btn-use-as-face">
        ...
        const generateAnotherBtn = this.resultPreview.querySelector('#btn-generate-another');
        ...
        const useAsFaceBtn = this.resultPreview.querySelector('#btn-use-as-face');
        if (useAsFaceBtn) {
    ...
    // Navigate to thumbnail view with a specific face image
    async generateFromGallery(imageId) {
        if (window.app) {
            app.showView('thumbnail');
        }
...
// Global YouTube thumbnail manager instance
let youTubeThumbnailManager;
